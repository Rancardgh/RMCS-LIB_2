...
public class KannelDriver extends PushDriver {
    private final Logger logger = Logger.getLogger(KannelDriver.class.getName());

    public KannelDriver(CPConnection cnxn, Map<String, String> params) throws IllegalArgumentException{
        super(cnxn, params);
    }

    @Override
    public String sendSMSTextMessage() throws IOException, IllegalArgumentException {
        ...
        final String url = cnxn.getGatewayURL() + "/sendsms?to=" + URLEncoder.encode(getParameterValue(params, "to"), "UTF-8")
                ...
                + "&smsc=" + URLEncoder.encode(getParameterValue(params, "smsc"), "UTF-8")
                + "&username=" + URLEncoder.encode(cnxn.getUsername(), "UTF-8")
                + "&password=" + URLEncoder.encode(cnxn.getPassword(), "UTF-8")
                + "&account=" + URLEncoder.encode(getParameterValue(params, "account"), "UTF-8")
                + "&meta-data=" + URLEncoder.encode("?smpp?keyword=" + getParameterValue(params, "keyword")
                + "&cpid=" + getParameterValue(params, "account_id") + "&product="+getParameterValue(params, "product"), "UTF-8");
