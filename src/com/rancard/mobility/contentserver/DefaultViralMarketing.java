...
public class DefaultViralMarketing extends ViralMarketing {
    ...
    public void sendHowToMessage(CPConnection cnxn, String msisdn, String smsc, String shortCode) {
        ...
        (new Thread(new ThreadedMessageSender(cnxn, msisdn, vmCampaign.getMessageSender(), howToMessage.replace("@@shortCode@@", shortCode), null,
                vmCampaign.getServiceDefinition().getServiceDefinitionKey().getAccountID(), vmCampaign.getServiceDefinition().getServiceDefinitionKey().getKeyword(),
                "RMCS", smsc, 0, 0))).start();
    }

    @Override
    public void updateVMTransaction(EntityManager em, CPConnection cnxn, String msisdn, String smsc) {
        if (vmCampaign == null || !vmCampaign.isActive()) {
            return;
        }

        VMTransaction transaction = VMTransaction.getVMTransaction(em, vmCampaign.getCampaignID(), msisdn);
        ...
        transaction.setStatus(VMTransactionStatus.INV_ACCEPTED);
        ...
        (new Thread(new ThreadedMessageSender(cnxn, transaction.getVmTransactionKey().getRecruiterMSISDN(), vmCampaign.getMessageSender(), invitationSuccess, null,
                vmCampaign.getServiceDefinition().getServiceDefinitionKey().getAccountID(), vmCampaign.getServiceDefinition().getServiceDefinitionKey().getKeyword(), "RMCS", smsc, 0, 0))).start();
    }
}
