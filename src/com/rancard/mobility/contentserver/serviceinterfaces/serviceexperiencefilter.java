...
import com.rancard.common.ThreadedMessageSender;
...
public class serviceexperiencefilter
        extends HttpServlet
        implements Filter {
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain filterChain)
        ...
        try {
            ...
            if (srvcExpr.exists()) {
                ...
                String hashString = msisdn + accountId + keyword.toUpperCase();
                promoImpressionsCheck(hashString.hashCode(), serviceRespCode, keyword, accountId, msisdn, request);
            } else {
    ...
    private void promoImpressionsCheck(long hashCode, String srvcRespCode, String keyword, String accountId, String msisdn, HttpServletRequest request) {
        try {
            System.out.println(new java.util.Date() + ": In promoImpression Checker");
            PromoImpression pImpression = VMServiceManager.viewPromoImpression(hashCode);
            ...
            if (!srvcRespCode.equals("")) {
                String hashString = msisdn + accountId + srvcRespCode.toUpperCase();
    ...
    private boolean bill(String msisdn, String accountID, String keyword, String site_id, HttpServletRequest request, String price, String shortcode) {
        ...
        try {
            ...
            String completeTransnxnUrl = fullContextPath + "/sendinfo_push.jsp?msisdn=" + URLEncoder.encode(msisdn, "UTF-8") + "&keyword=" + URLEncoder.encode(keyword.toUpperCase(), "UTF-8") + "&alert_count=" + is.getMsgId() + "&dest=" + URLEncoder.encode(shortcode, "UTF-8") + "&siteId=" + URLEncoder.encode(site_id, "UTF-8") + "&transId=" + URLEncoder.encode(transactionId, "UTF-8");
    ...
    private int updateSubscription(String msisdn, String accountId, String keyword, int subscriptionInterval, boolean activeSub)
            throws Exception {
    ...
    private boolean isInActiveUser(String accountId, String keyword, String msisdn) {
        ...
        try {
            ...
            try {
                ...
                Logger.getLogger(serviceexperiencefilter.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
        return status == -1;
    }
