...
<c:set var="isUserAdded" value="false" />
<c:if test="${param['action']=='addnewuser'}">
    ...
    <sql:transaction>
     <sql:query var="msisdn_lookup">
         select * from users where msisdn =?
        <sql:param value="${param['msisdn']}"/>       
     </sql:query>
        <c:choose>
            <c:when test="${ empty msisdn_lookup.rows}">
            ...
            <c:set var="bank_sel" value="${param['bank']}"/>
            <c:set var="network_sel" value="${param['network']}"/>
            <c:choose>
            ...
            <c:set var="isUserAdded" value="true" />
        
                </c:when>
            <c:otherwise>
                <c:set var="userAddedMsg" value="MDISDN Exists for another user" />
            </c:otherwise>
        </c:choose>
                        ...
                        <td width="99%" id="jive-content">
                            ...
                            <c:choose>
                                <c:when test="${isUserAdded=='true'}">
                                     User <a href ="user-details.jsp?id=${cikanet_acc_no}"> ${param['firstname']} ${param['lastname']} </a> sucessfully added<BR><br>
                                ...
                                <c:otherwise>
                                    ...
                                    <B> ${userAddedMsg} <B><BR><br>
                                    </c:if>
                                       <form id="form1" name="form1" method="post" action="new_user.jsp?action=addnewuser&useraction=4">
                                            ...
                                            <tr>
                                                ...
                                                <td><select name="network" onChange="MM_jumpMenu('parent',this,0)">
                                                        ...
                                                        <c:forEach var="row" items="${networks.rows}">
                                                            <option value =<c:out value="${row.network_id}"/>>
